...
private Quaternion flashlight_aim_rot;
		...
		void FieldInitialisers(){
				aim_spring = new Spring(0,0,kAimSpringStrength,kAimSpringDamping);
				flashlight_mouth_spring= new Spring(0,0,kAimSpringStrength,kAimSpringDamping);
				flash_ground_pose_spring= new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				 x_recoil_spring = new Spring(0,0,kRecoilSpringStrength,kRecoilSpringDamping);
				 y_recoil_spring = new Spring(0,0,kRecoilSpringStrength,kRecoilSpringDamping);
				 head_recoil_spring_x = new Spring(0,0,kRecoilSpringStrength,kRecoilSpringDamping);
				 head_recoil_spring_y = new Spring(0,0,kRecoilSpringStrength,kRecoilSpringDamping);
				 slide_pose_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				 reload_pose_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				 press_check_pose_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				 inspect_cylinder_pose_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				 add_rounds_pose_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				 eject_rounds_pose_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				 hold_pose_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				 mag_ground_pose_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
				head_recoil_delay = new float[(int)kMaxHeadRecoil];
				 show_bullet_spring = new Spring(0,0,kAimSpringStrength, kAimSpringDamping);
		}
		...
		Quaternion flash_ground_rot = new Quaternion();
		...
		void Start() { 
				...
				renderers = gun_instance.GetComponentsInChildren<Renderer>();
				...
				if(GetGunScript().gun_type == GunScript.GunType.AUTOMATIC){
				...
				List<GameObject> loose_bullets = new List<GameObject>();
				List<GameObject> loose_bullet_spring = new List<GameObject>();
		...
		if(count >= 2){
			break;
				...
				while(temp_total_tapes.Count > 0){
						var rand_tape_id = Random.Range(0,temp_total_tapes.Count-1);
		...
		void HandleGunControls(bool insert_mag_with_number_key) {
				...
				if(gun_script.add_head_recoil){
						...
						next_head_recoil_delay = (int)((next_head_recoil_delay + 1)%kMaxHeadRecoil);
		...
		void StartWin() {
				GetComponent<MusicScript>().HandleEvent(MusicScript.MusicEvent.WON);
				won = true;
		}

		void ApplyPose(string _name,float amount){
				Transform pose = gun_instance.transform.FindChild(_name);
				if(amount == 0.0f || !pose){
						return;
				}
		...
		void UpdateCameraAndPlayerTransformation() {
				...
				main_camera.transform.position += new Vector3(0, character_controller.height * character_controller.transform.localScale.y - 0.1f, 0);
				main_camera.transform.position +=  new Vector3(0, head_fall, 0);
		}
		...
		void UpdateGunTransformation() {
				...
				var unaimed_dir = (transform.forward -Vector3.up).normalized;
				...
				if(!disable_recoil){		
						gun_instance.transform.RotateAround(
								...
								gun_instance.transform.rotation * Vector3.right,
		...
		void UpdateInventoryTransformation() {
				...
				for(i=0; i<10; ++i){
						...
						renderers = slot.obj.GetComponentsInChildren<Renderer>();
						foreach(Renderer renderer in renderers){
								renderer.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off; 
						}
						slot.spring.Update();
		...
		void UpdateLooseBulletDisplay() {
				bool revolver_open = (gun_instance && gun_instance.GetComponent<GunScript>().IsCylinderOpen());
				...
				for(int i=0; i<loose_bullets.Count; ++i){
						AimScript.Spring spring = loose_bullet_spring[i];
						spring.Update();
						GameObject bullet = loose_bullets[i];
						bullet.transform.position = main_camera.transform.position + main_camera.GetComponent<Camera>().ScreenPointToRay(new Vector3(0, main_camera.GetComponent<Camera>().pixelHeight,0)).direction * 0.3f;
						...
						bullet.transform.localScale = Vector3.one*spring.state;
		...
		void UpdatePickupMagnet() {
				Vector3 attract_pos = transform.position - new Vector3(0,character_controller.height * 0.2f,0);
				for(int i=0; i<collected_rounds.Count; ++i){
						GameObject round = collected_rounds[i] as GameObject;
						if(!round){
								continue;
						}
		...
		void OnGUI() {
				...
				if(!show_help){
						...
						if(gun_instance){
								...
								if(gun_script.gun_type ==GunScript.GunType.REVOLVER){
